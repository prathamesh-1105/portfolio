...
export function Hero() {
  ...
  const micOpacity = useTransform(scrollY, [0, 80, 150, 300], [0, 1, 1, 0]);
  const micY = useTransform(scrollY, [80, 300], [0, 200]);
  const micRotate = useTransform(scrollY, [80, 300], [0, 30]);
  const micClipY = useTransform(scrollY, [120, 300], [0, 100]);
  ...
  return (
    <section className="relative min-h-screen w-full flex items-center overflow-hidden bg-background">
      ...
      >
        ...
        {/* Holographic Microphone Overlay Container */}
        <div 
          className="absolute top-[53.3%] left-[75.0%] -translate-x-1/2 -translate-y-1/2 w-[60%] aspect-square z-30"
        >
          {/* Microphone SVG */}
          <motion.svg
            viewBox="0 0 100 100"
